...
import type { startExecData, getExecStatusData, killExecData } from '@interfaces/IExecution.js';
...
class ExecutionModel {
  ...
  private conn_obj!: ConnHandlerImpl;
  ...
  constructor({ exec_token, env, threshold, verbose }: { exec_token: string; env?: string; threshold?: number; verbose?: boolean }) {
  ...
  public async startExec(): Promise<any> {
    try {
      this.conn_obj = new ConnHandlerImpl();
      this.trigger_payload = {
        token: this.exec_token,
      };
      this.setReqHeader();
      if (this.verbose_flag) {
        logger.info(`\nAPI CALLED: ${JSON.stringify(this.build_api)}`);
        logger.info(`REQUEST BODY: ${JSON.stringify(this.trigger_payload)}`);
        logger.info(`REQUEST BODY: ${JSON.stringify(this.trigger_payload)}`);
      }

      const resp: AxiosResponse | AxiosError | Error = await this.conn_obj.makePostRequest({
        url: this.build_api,
      ...
      if (axios.isAxiosError(resp)) {
        if (resp.response) {
          ...
          if (status_code >= 400 && status_code < 600) {
            ...
            const statusMessage = statusMessages[status_code] || resp.response.statusText;
          ...
          logger.error(`EXECUTION STATUS: No response received. Is the server down?`);
          return null;
        } else {
          logger.error(`EXECUTION STATUS: Something is critically broken on SQA Servers.`);
  ...
  public async checkExecStatus({ payload_flag }: { payload_flag: boolean }): Promise<any> {
    try {
      this.conn_obj = new ConnHandlerImpl();
      this.status_payload = {
      ...
      if (this.verbose_flag && payload_flag) {
        logger.info(`\nAPI CALLED: ${JSON.stringify(this.status_api)}`);
        logger.info(`\nREQUEST BODY: ${JSON.stringify(this.status_payload)}`);
      }
      ...
      if (axios.isAxiosError(resp)) {
        ...
        if (response_data === null) {
          ...
          response_data.data.data.result.forEach((item: { result: string }) => {
            if (item.result.toUpperCase() === 'FAILED') this.tcs_failed = this.tcs_failed + 1;
          });
  ...
  public async killExec(): Promise<any> {
    try {
      ...
      if (this.verbose_flag) {
        logger.info(`\nREQUEST BODY: ${JSON.stringify(this.kill_payload)}`);
      }
      ...
      if (axios.isAxiosError(resp)) {
        if (resp.response) {
          ...
          if (status_code >= 400 && status_code < 600) {
            const statusMessage = statusMessages[status_code] || resp.response.statusText;
            logger.error(`EXECUTION STATUS: Status code ${status_code}, Execution did not kill successfully.`);
            logger.error(`REASON OF FAILURE: ${statusMessages[status_code]} ${this.app_url}`);
          }
          return null;
        } else if (resp.request) {
